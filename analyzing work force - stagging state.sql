...
(select survey.`Engagement Score`,  COUNT(CASE WHEN `attrition flag` = 1 THEN employee.`Employee Id` END) * 100.0 / COUNT(*) AS Engagement_attrition_rate,count(employee.`Employee Id`) con
...
,CompanyAttrition as 
(
select COUNT(CASE WHEN `attrition flag` = 1 THEN `Employee Id` END) * 100.0 / COUNT(*) AS company_avg_attrition
...
(
SELECT `Engagement Score` ,count(CASE WHEN `attrition flag`=1 then edc.`Employee ID`end) * 100 / count(*) as engagement_attrition
...
(select `Department`,  COUNT(CASE WHEN `attrition flag` = 1 THEN `Employee Id` END) * 100.0 / COUNT(*) AS Department_attrition_rate,
...
(select `Performance Score`,Department, 
			COUNT(CASE WHEN `attrition flag` = 1 THEN employee.`Employee Id` END) * 100.0 / COUNT(*) AS performance_attrition_rate,
...
( 
select 
	COUNT(CASE WHEN `attrition flag` = 1 THEN `Employee Id` END) * 100.0 / COUNT(*) companyattrition
from second_project.employee_data_cleaned
),jobrole as
(
select t.year,
...
(select `Job Role`, 
		COUNT(CASE WHEN `attrition flag` = 1 THEN `Employee Id` END) * 100.0 / COUNT(*) AS jobrole_attrition_rate_left,
		COUNT(CASE WHEN `attrition flag` = 1 THEN `Employee Id` END) employee_left,
        COUNT(CASE WHEN `attrition flag` = 0 THEN `Employee Id` END) * 100.0 / COUNT(*) AS jobrole_attrition_rate_stay,
...
select `Satisfaction Score`,count(case when `attrition flag` =1 then dat.`Employee ID` end)*100 /count(*) leftemployee_satisfaction,
							count(case when `attrition flag` =1 then dat.`Employee ID` end) employee_left,
                            count(case when `attrition flag` =0 then dat.`Employee ID` end)*100 /count(*) stayemployee_satisfaction,
...
(
select count(case when `attrition flag` =1 then `Employee ID` end)*100 /count(*) companyattrion
from second_project.employee_data_cleaned
...
select `Work-Life Balance Score`,Department,count(case when `attrition flag` =1 then dat.`Employee ID` end)*100 /count(*) leftworklifebalance_Score,
							count(case when `attrition flag` =1 then dat.`Employee ID` end) employee_left,
                            count(case when `attrition flag` =0 then dat.`Employee ID` end)*100 /count(*) stayworklifebalance_Score,
							count(case when `attrition flag` =0 then dat.`Employee ID` end) employee_stay
